...
public class ExecutionTest {
    ...
    /**
     * 동적으로 실제 파라미터로 넘어온 객체 인스턴스로 판단. args() 상위 타입 가능.
     * 정적으로 클래스에 선언된 정보만 보고 판단. execution()에서 타입 매칭 시 정확하게 매칭해야함. 다형성 사용 x
     */

    /**
     * @target : 인스턴스 기준
     * @within : 해당 인스턴스만. 부모 타입 x
     * args, @args, @target 지시자는 단독으로 사용하면 안된다.
     * 해당 지시자는 실제 객체 인스턴스가 생성되고 실행될 떄 어드바이스 적용 여부를 확인할 수 있다. 런타임에 적용된다는 건 실제 인스턴스가 만들어 진 후에 판단하는 것. -> 스프링은 일단 프록시를 다 만들어버림..
     * 스프링이 모든 스프링 빈에 aop를 적용하려 시도함.
